"""
Memory-Efficient Knowledge Distillation Loss for Distill-3R (No Confidence Weighting)

ABLATION VERSION: This version removes confidence weighting from geometric losses
to test the necessity of uncertainty weighting for training stability.
...
class DistillLossNoConfWeight(nn.Module):
    """
    Knowledge distillation loss WITHOUT confidence weighting (ablation study).
    ...
    ABLATION: Removes confidence weighting from geometric losses to test if
    uncertainty weighting is necessary for stable training.
    """
    ...
    def _confidence_weighted_loss(self, pred: torch.Tensor, target: torch.Tensor,
        ...
        """
        Compute L2 loss WITHOUT confidence weighting (ablation study).

        ABLATION: This version removes confidence weighting to test if uncertainty
        weighting is necessary for stable training and good performance.

        Args:
            pred: [B, H, W, 3] or [B, H, W] predictions (single view)
            target: [B, H, W, 3] or [B, H, W] targets (single view)
            confidence: [B, H, W] teacher confidence scores (ignored in this ablation)
        ...
        # ABLATION: Skip confidence weighting to test necessity
        # Original code:
        #   conf_weight = torch.clamp(confidence / 10.0, min=0.1, max=1.0)
        #   loss = loss * conf_weight
        # Now: No confidence weighting applied
