"""
Memory-Efficient Knowledge Distillation Loss for Distill-3R (Student Confidence Weighting)

ABLATION VERSION: This version uses STUDENT confidence to weight geometric losses
instead of teacher confidence, mimicking how Fast3R trains with self-supervised weighting.
...
class DistillLossStudentConfWeight(nn.Module):
    """
    Knowledge distillation loss using STUDENT confidence weighting (ablation study).

    ABLATION: Uses student's predicted confidence to weight geometric losses
    instead of teacher confidence. Tests whether student's own uncertainty estimates
    provide sufficient training signal compared to teacher's confidence.
    ...
    def _global_loss(self, pred: Dict, target: Dict, mask: torch.Tensor) -> torch.Tensor:
        """
        Global loss with cross-view normalization using STUDENT confidence weighting.

        ABLATION: Uses student's predicted confidence instead of teacher's confidence
        to weight the geometric loss, mimicking Fast3R's self-supervised training.

        All views normalized by SINGLE scale factor to enforce consistent alignment.
        """
        if 'conf_global' not in pred:
            raise ValueError("conf_global missing from pred (student predictions)")
        ...
        # ABLATION: Use STUDENT confidence instead of teacher confidence
        total_loss = 0.0
        for i in range(N):
            conf = pred['conf_global'][:, i]  # [B, H, W] - STUDENT confidence
    ...
    def _local_loss(self, pred: Dict, target: Dict, mask: torch.Tensor) -> torch.Tensor:
        """
        Local loss with per-view normalization using STUDENT confidence weighting.
        ...
        if 'conf_local' not in pred:
            raise ValueError("conf_local missing from pred (student predictions)")
        ...
        total_loss = 0.0
        for i in range(N):
            conf = pred['conf_local'][:, i]  # [B, H, W] - STUDENT confidence
            view_loss = self._confidence_weighted_loss(
                pred_pts_list[i], targ_pts_list[i],
                conf, valid_list[i], weight=1.0
            )
